# File overview: api/python/load_profile.py
# Purpose: models load-vs-solar overlap and derives self-consumption ratios.
import math

# Normalized 24-hour demand archetypes by tariff category.
PROFILES = {
...
# Typical daylight-heavy solar generation shape over 24 hours.
...
def _normalize(values: list[float]) -> list[float]:
    """Scale values so they sum to 1.0 while preserving relative shape."""
    total = sum(values)
    if total <= 0:
...
def compute_self_consumption(tariff_category: str) -> dict:
    """Estimate direct-use vs export ratio by overlapping load and solar curves."""
    # Normalize user-supplied category and fallback to domestic when unknown.
    ...
    # Convert both curves into comparable hourly proportions.
    load = _normalize(PROFILES[normalized_category])
    solar = _normalize(SOLAR_CURVE)

    # Hourly overlap defines solar that can be self-consumed on site.
